...
colors.push(
  ...
  {id:'yG', color:'#e6c200', text:'Золотистый',         tone:'тёплый', style:'элегантный',season:'осень', colortype:'осень', textureHint:'Парча',      makeupHint:'Хайлайтер золотой',   makeupSet:{lips:'Коралловая', eyes:'Золотистые', blush:'Тёплые', eyeliner:'Коричневая', style:'элегантный'}, hue:68},
...
colors.push(
  {id:'pvT', color:'#d1b3ff', text:'Светло-фиолетовый',  tone:'холодный', style:'романтичный',season:'лето',colortype:'лето',textureHint:'Шифон',    makeupHint:'Тени светло-фиолетовые',makeupSet:{lips:'Розовая', eyes:'Сиреневые', blush:'Светлые', eyeliner:'Серая', style:'романтичный'}, hue:242},
  ...
  {id:'pvA', color:'#6a0dad', text:'Фиолетовый насыщенный',tone:'холодный',style:'драматичный',season:'зима',colortype:'зима',textureHint:'Бархат',    makeupHint:'Тени тёмно-фиолетовые',makeupSet:{lips:'Пурпурная', eyes:'Фиолетовые', blush:'Холодные', eyeliner:'Чёрная', style:'вечерний'}, hue:248},
...
colors.push(
  {id:'rpT', color:'#ffb3f5', text:'Светло-розовый фиолет',tone:'холодный', style:'романтичный',season:'лето',colortype:'лето',textureHint:'Шифон',  makeupHint:'Помада светло-розовая',makeupSet:{lips:'Светло-розовая', eyes:'Сиреневые', blush:'Розовые', eyeliner:'Серая', style:'романтичный'}, hue:302},
...
colors.push(
  ...
  {id:'nLatte', color:'#c5a582', text:'Латте', tone:'тёплый', style:'повседневный', season:'все', colortype:'универсальный', textureHint:'Кашемир', makeupHint:'Хайлайтер золотой', makeupSet:{lips:'Нюдовая', eyes:'Золотистые', blush:'Тёплые', eyeliner:'Коричневая', style:'повседневный'}, hue:45, virtualHue:45, baseTone:'warm'},
...
romantic: (i, colors = colors) => {
    ...
    // 1. Сначала ищем цвета стиля "романтичный"
    const romanticStyleColors = colors.filter(color => 
        color.style === "романтичный" && 
        color.id !== baseColor.id
    );
...
const schemeInfo = {
  ...
  romantic: {
    ...
    paletteRule: "Цвета со стилем 'романтичный'."
  },
...
// Проверка стиля элемента
function checkStyleFilter(item) {
    const filter = currentFilters.style;
    if (!filter) return true;
    
    const itemStyle = (item.dataset.styles || '').toLowerCase();
    if (!itemStyle) return false;
    
    // Если фильтр содержит пробел или запятую (комбинированный)
    if (filter.includes(' ') || filter.includes(',')) {
        const filters = filter.split(/[, ]+/).map(f => f.trim().toLowerCase());
        
        for (let filterStyle of filters) {
            // Прямое совпадение
            if (itemStyle === filterStyle) return true;
    ...
    // Одиночный фильтр
    return itemStyle === filter.toLowerCase();
}
...
function resetFilters() {
    ...
    document.querySelectorAll('.card, .item, [data-styles]').forEach(item => {
...
// Остальные функции можно удалить, так как они дублируются:
// - getItemStyles(item) - не используется в новой реализации
// - вторая applyFilters() - удалена, оставлена основная

// === БОЛЬШОЙ КРУГ С GLOW ЭФФЕКТОМ И ФИЛЬТРАЦИЕЙ ===
function renderColorWheel() {
    ...
    colors.forEach((color, index) => {
        ...
        // ПРОВЕРЯЕМ ВИДИМОСТЬ ПО ФИЛЬТРАМ
    ...
    // ВАШ ПОЛНЫЙ GLOW ЭФФЕКТ для выбранного цвета
    if (lastChosenIndex !== null && colors[lastChosenIndex]) {
        ...
        // Проверяем видимость выбранного цвета
